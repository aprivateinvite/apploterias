...
def main(page: ft.Page):
    ...
    def obter_diretorio_exportacao():
        # 1. Verifica pastas públicas no Android (visíveis diretamente em "Meus Arquivos")
        caminhos_android = [
        ...
        # 2. No Computador (Windows/Linux/Mac), usa Downloads ou Documentos do usuário
        home = os.path.expanduser("~")
        for subpasta in ["Downloads", "Documents", "Documentos"]:
            caminho_pc = os.path.join(home, subpasta)
            if os.path.exists(caminho_pc) and os.path.isdir(caminho_pc):
                return caminho_pc

        # 3. Fallback seguro
        return home if os.path.exists(home) else os.getcwd()
    ...
    def btn_exportar_click(e):
        ...
        try:
            diretorio = obter_diretorio_exportacao()
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            nome_arquivo = f"combinacoes_loteria_{timestamp}.txt"
            ...
            with open(arquivo_path, "w", encoding="utf-8") as f:
                ...
                for idx, item in enumerate(resultados_sessao, 1):
                    f.write(f"Jogo #{idx} - Modalidade: {item['modalidade'].capitalize()}\n")
                    f.write(f"Dezenas: {', '.join(item['dezenas'])}\n")
                    if item['trevos']:
                        f.write(f"Trevos: {', '.join(item['trevos'])}\n")
                    f.write("-" * 35 + "\n")

            mostrar_snack(f"Salvo com sucesso em: {arquivo_path}")
        except Exception as ex:
